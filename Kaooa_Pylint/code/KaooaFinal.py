...
def check_click_crow(x, y):
    ...
    for point in crows:
        ...
        if distance <= 60:
            ...
            turtle.onscreenclick(check_click)   # pylint: disable=maybe-no-member
...
def check_click(x, y):
    ...
    turtle.onscreenclick(None)  # pylint: disable=maybe-no-member
    global FLAG
    global NUM
    global KILLED

    for point in CLICKED_POINTS: # pylint: disable=too-many-nested-blocks
            ...
            if IS_PRESENT[index] == 0 and TURN == 1:
                ...
                if vulture.xcor() != -377:
                    ...
                    if CLICKED_POINTS.index(point) in VALID_LIST[index]:
                        ...
                        for z in common:
                            for r in VALID_LIST[z]:
                                if r in VALID_LIST[index] and IS_PRESENT[y] == 1 and z == CLICKED_POINTS.index(point):# pylint: disable=line-too-long
                                    KILLED += 1
                                    if KILLED > 3:
    ...
    if NUM % 2 == 0:
        turtle.onscreenclick(check_click_crow)    # pylint: disable=maybe-no-member
    else:
        turtle.onscreenclick(check_click_vulture)    # pylint: disable=maybe-no-member
...
def crows_winner():
    ...
    for i in VALID_KILL[CLICKED_POINTS.index((vulture.xcor(), vulture.ycor()))]:
        if IS_PRESENT[i] == 1:
            crowcount2 += 1
    if (crowcount == 2 and CLICKED_POINTS.index((vulture.xcor(), vulture.ycor())) in [1, 2, 7, 8, 9] and crowcount2 == 2)  or (crowcount == 4 and CLICKED_POINTS.index((vulture.xcor(), vulture.ycor())) in [0, 3, 4, 5, 6] and crowcount2 == 2): # pylint: disable=line-too-long  too-many-boolean-expressions
        print("Crows win")
        sys.exit()
    else:
        turtle.onscreenclick(check_click_crow)     # pylint: disable=maybe-no-member
